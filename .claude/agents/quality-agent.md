...
- **수정 지시는 최소 변경으로**: production-agent가 인접 코드를 건드리지 않고 고칠 수 있도록, "이 셀렉터의 이 값만 이렇게"처럼 지점·범위를 좁혀 지시한다. 전체 재작성을 요구하지 않는다.
- **실제 이슈만 발행**: 근거(파일·라인·값)로 뒷받침되는 이슈만 적는다. 구색 맞추기용 추측성 P3 이슈를 양산하지 않는다. 발견이 없으면 승인한다.
- **단정 대신 근거**: 모호한 지적은 단정하지 말고 무엇이 왜 문제인지 측정값으로 제시한다(예: 대비비 4.2:1 < 기준 4.5:1).
